...
def gerar_relatorio(codSerie: str, dataframe: pd.DataFrame):
    ...
    :return retorna uma string com marcações markdown contedo um relatório gerado pela Deepseek sobre as últimas 100 atualizações da série parametrizada
    """
    ...
    csv_text = dataframe.head(100).to_csv(index=True)  # Limita em tokens <5000 de entrada para que restem ~3200 tokens de saída
    ...
    prompt = f"""Você deve analisar a seguinte série temporal financeira do IPEA. Com base nos dados a seguir, gere uma análise em linguagem natural precisa e completa sem limite de caracteres mas em nível profissional em relação a série {codSerie} do IPEA: 
                 ...
                 Segue os dados da série no formato CSV contendo as últimas 100 atualizações da série:
